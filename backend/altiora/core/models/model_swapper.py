...
class ModelSwapper:
    ...
    async def _load_doctopus(self, name: str, config: Dict[str, Any]) -> None:
        """Charge DocToPus (service OCR)."""
        try:
            # Import direct du wrapper OCR depuis services
            from backend.altiora.services.ocr.doctopus_ocr import DoctopusWrapper

            # Configuration
            config_path = config.get("config_path", "/app/config/doctopus_config.json")
            wrapper = DoctopusWrapper(config_path=config_path)

            # Stockage direct du wrapper
            self._models[name] = wrapper
            self._managers[name] = wrapper

            logger.info("DocToPus OCR chargé avec succès")

        except Exception as e:
            logger.error(f"Erreur chargement DocToPus: {e}")
            # Fallback sur mock
            self._models[name] = "doctopus_mock"
            logger.warning("DocToPus en mode mock suite à l'erreur")
    ...
    @staticmethod
    def _check_memory(required_gb: float) -> bool:
        """
